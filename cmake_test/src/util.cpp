...
{
    assert(start >= 0 && end < options.all_vertex.size());
    assert(end < static_cast<uint64_t>(options.values.size()));

    for (uint64_t i = start; i <= end; ++i) {
        if (is_vertex) {
            rocksdb::Status s = options.cf_partition->Put(
                write_options, cf_id, options.all_vertex[i]->GetRep(),
                options.values[i]);
            if (!s.ok()) return s;
        }
        else {
            manager::Edge *edge =
                (is_out_edge) ? options.in_edge_map_to_out_edge[i].first
                              : options.in_edge_map_to_out_edge[i].second;

            rocksdb::Status s = options.cf_partition->Put(
                write_options, cf_id, edge->GetRep(), options.values[i]);

            if (save_edge)
                options.edge_key_map_to_value[edge->GetRep()] =
                    options.values[i];
                          ...
                          bool is_out_edge, bool save_edge)
{
    ...
    for (uint64_t i = start; i <= end; ++i) {
        if (is_vertex) {
            rocksdb::Status s = options.db_partition->Put(
                write_options, db_id, options.all_vertex[i]->GetRep(),
                options.values[i]);
            if (!s.ok()) return s;
        }
        else {
            ...
            rocksdb::Status s = options.db_partition->Put(
                write_options, db_id, edge->GetRep(), options.values[i]);
                   ...
                   const std::vector<std::string> &result)
{
    for (uint64_t i = start; i <= start; ++i) {
        assert(result[i] == options.values[i]);
    }
}
                              ...
                              uint64_t start, uint64_t end, bool need_verify)
{
    ...
    for (uint64_t i = start; i <= end; ++i) {
        rocksdb::Status s =
            options.cf_partition->GetVertex(read_options, cf_id, i, &value);
                   ...
                   const std::string &src_value, bool get_out_edge)
{
    manager::Vertex src_vertex(src_vertex_id,
                               options.manager_options.vertex_options_);
    ...
    assert(options.edge_key_map_to_value[edge.GetRep()] == src_value);
}
...
{
    assert(src_vertex_id >= 0 && src_vertex_id < options.all_vertex.size());
    assert(dest_vertex_id >= 0 && dest_vertex_id < options.all_vertex.size());

    rocksdb::Status s;
    std::string value;
    if (get_out_edge)
        s = options.cf_partition->GetOutEdgeByVertex(
            read_options, cf_id, src_vertex_id, dest_vertex_id, &value);
    else
        s = options.cf_partition->GetInEdgeByVertex(
            read_options, cf_id, src_vertex_id, dest_vertex_id, &value);
                                       ...
                                       bool get_out_edge, bool need_verify)
{
    ...
    std::string value;
    if (get_out_edge)
        s = options.db_partition->GetOutEdgeByVertex(
            read_options, db_id, src_vertex_id, dest_vertex_id, &value);
    else
        s = options.db_partition->GetInEdgeByVertex(
            read_options, db_id, src_vertex_id, dest_vertex_id, &value);
                                  ...
                                  manager::Vertex *vertex, int build_all)
{
    switch (build_all) {
        case 1: {
            options.vertex_manager->ThisVertexAsSrcVertex(
                vertex, &options.in_edge_map_to_out_edge);
            break;
        }
        case 2: {
            options.vertex_manager->ThisVertexAsDestVertex(
                vertex, &options.in_edge_map_to_out_edge);
            break;
        }
        default: {
